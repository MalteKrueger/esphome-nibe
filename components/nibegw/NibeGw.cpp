    ...
    case STATE_WAIT_DATA:
      if (RS485->available() > 0) {
        ...
        if (index >= MAX_DATA_LEN) {
          ...
          ESP_LOGVV(TAG, "checkMsg=%hhu", msglen);
          //ESP_LOGD(TAG, "checkMsg=%hhu", msglen);
...
//#if ESPHOME_LOG_LEVEL >= ESPHOME_LOG_LEVEL_VERY_VERBOSE
            ...
            ESP_LOGD(TAG, "CRC Error for Message of %hhu bytes received from heat pump: %s", index, debug_buf);
#endif
      state = STATE_WAIT_START;
      break;
...
//#if ESPHOME_LOG_LEVEL >= ESPHOME_LOG_LEVEL_VERY_VERBOSE
  ...
  ESP_LOGD(TAG, "Sent message of %hhu bytes to heat pump: %s", len, debug_buf);
#endif
}
